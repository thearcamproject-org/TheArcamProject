  ...
  return (
    <section ref={ref} id="services" className="relative py-20 md:py-32 bg-black overflow-hidden antialiased">
      ...
      <div className="w-full px-4 md:px-10 relative z-10">
        ...
        {/* Services Grid - Optimized for Mobile Comparison */}
        <motion.div
          ...
          {SERVICE_TIERS.map((service, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={`relative shadow-2xl ${shades[index % 4]} border border-white/5 px-4 md:px-6 py-8 md:py-12 h-full overflow-hidden rounded-[2rem] md:rounded-[3rem] flex flex-col justify-start items-start hover:border-[#E7B366]/20 transition-colors duration-700 group backdrop-blur-3xl min-h-[450px] md:min-h-[600px]`}>
                  {service.popular && (
                    <div className="absolute top-4 md:top-8 right-4 md:right-10">
                      <span className="px-2 md:px-4 py-1 rounded-full bg-[#E7B366] text-black text-[7px] md:text-[9px] font-black uppercase tracking-[0.2em] shadow-2xl shadow-[#E7B366]/20">Popular</span>
                    </div>
                  )}
                  ...
                  <h3 className="font-medium text-lg md:text-2xl lg:text-3xl text-white mb-1 md:mb-3 relative z-50 tracking-tight" style={{ fontFamily: "var(--font-serif)" }}>
                    {service.title}
                  </h3>

                  <p className="font-semibold text-[8px] md:text-[10px] text-[#E7B366] mb-4 md:mb-6 relative z-50 leading-relaxed uppercase tracking-[0.3em]">
                    {service.subtitle}
                  </p>

                  {service.description && (
                    <p className="text-[10px] md:text-sm text-white/50 mb-6 md:mb-8 relative z-50 font-light leading-relaxed line-clamp-3 md:line-clamp-none">
                      {service.description}
                    </p>
                  )}
                  ...
                  <div className="mt-auto w-full pt-4 md:pt-8 border-t border-white/5 relative z-50">
                    <div className="mb-4 md:mb-6">
                      <p className="text-[8px] md:text-[10px] text-[#E7B366] uppercase tracking-[0.2em] font-semibold mb-1">Starting at</p>
                      <div className="flex items-baseline gap-1">
                        <span className="text-xl md:text-4xl font-light text-white" style={{ fontFamily: "var(--font-serif)" }}>₹{service.price.toLocaleString('en-IN')}</span>
                      </div>
                    </div>

                    <motion.a
                      href={`/configure?tier=${service.id}`}
                      className="w-full group/btn relative py-3 md:py-5 rounded-lg md:rounded-xl border border-[#E7B366]/20 bg-white/5 overflow-hidden transition-all duration-500 hover:border-[#E7B366]/40 hover:shadow-[0_0_20px_rgba(231,179,102,0.1)] flex items-center justify-center"
                    >
                      ...
                      <span className="relative z-10 text-white/80 group-hover:text-black font-semibold text-[8px] md:text-[10px] tracking-[0.2em] md:tracking-[0.4em] uppercase transition-all duration-500 text-center">
                        Acquire
                      </span>
                    </motion.a>
                  </div>
